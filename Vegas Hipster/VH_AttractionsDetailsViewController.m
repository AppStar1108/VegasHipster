...
@interface VH_AttractionsDetailsViewController ()

@end

@implementation VH_AttractionsDetailsViewController
...
- (void)get_table_data {
	...
	NSString *query = [[NSString alloc] initWithFormat:@"SELECT name, hotel_id, showroom, phone, description, image_path FROM attractions_db WHERE attractions_id = %@;", workingId];
    NSLog(@"%@", query);
	if (sqlite3_prepare_v2(db, [query UTF8String], -1, &statement, nil) == SQLITE_OK) {
		while (sqlite3_step(statement) == SQLITE_ROW) {
			...
			if (rest_image == NULL || [rest_image isEqualToString:@""]) {
				header_image = [UIImage imageNamed:@"hdrAttractions.jpg"];
			}else{
...
- (void)shareThis:(id)sender {
    ...
    [AddThisSDK presentAddThisMenuForURL:linkUrl withTitle:linkTitle description:@"Check out this attraction I've found using Vegas Hipster's iPhone app!"];
}
...
-(void)downloadImage:(id)image_path {
    ...
    NSString *hotelsPath = [libraryDirectory stringByAppendingPathComponent:@"images/attractions"];
    NSString *myFilePath = [libraryDirectory stringByAppendingPathComponent:image_path];
...
-(void)dialPhone:(id)sender {
    ...
    NSLog(@"Phone Number: %@", strippedString);
...
- (void)get_gallery_count {
	NSString *NSURLString = [[NSString alloc] initWithFormat:@"http://www.vegashipster.com/mobile.php?action=gallery_count&type=attractionId&id=%@", workingId];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([[segue identifier] isEqualToString:@"pushMapView"]) {
        ...
        [vc setWorkingVariables:@"attractions_db" withIdentifier:@"attractions_id" withId:workingId withUrl:theUrl];

//
//  VH_ClubsDetailsViewController.m
//  Vegas Hipster
//
...
@interface VH_ClubsDetailsViewController ()

@end

@implementation VH_ClubsDetailsViewController
...
- (void)get_table_data {
	...
	NSString *query = [[NSString alloc] initWithFormat:@"SELECT name, hotel_id, location, phone, description, image_path FROM clubs WHERE clubs_id = %@;", workingId];
	...
	if (sqlite3_prepare_v2(db, [query UTF8String], -1, &statement, nil) == SQLITE_OK) {
		while (sqlite3_step(statement) == SQLITE_ROW) {
			...
			NSString *clubs_location = [NSString stringWithUTF8String:(char *)sqlite3_column_text(statement, 2)];
            ...
            NSString *club_image = [NSString stringWithUTF8String:(char *)sqlite3_column_text(statement, 5)];
            
			if (club_image == NULL || [club_image isEqualToString:@""]) {
				header_image = [UIImage imageNamed:@"hdrClubs.jpg"];
			}else{
				NSString *image_path = [club_image stringByReplacingOccurrencesOfString:@"../" withString:@""];
            ...
            [listClubsArray addObject:[[NSMutableDictionary alloc] initWithObjectsAndKeys:name, @"name",
                                      hotel_id, @"hotel_id",
                                      clubs_location, @"location",
                                      phone, @"phone",
        ...
        NSString *query2 = [[NSString alloc] initWithFormat: @"SELECT name, address1, address2 FROM hotel_db WHERE hotel_id = %@;", [[listClubsArray objectAtIndex:0] objectForKey:@"hotel_id"]];
        ...
        if (sqlite3_prepare_v2(db, [query2 UTF8String], -1, &statement2, nil) == SQLITE_OK) {
            ...
            NSLog(@"Database Error 2 = %s", sqlite3_errmsg(db));
        }
        //	sqlite3_finalize(statement5);
        sqlite3_finalize(statement2);
	}else{
        //NSLog(@"Can't connect to the db...");
        NSLog(@"Database Error 1 = %s", sqlite3_errmsg(db));
    }
...
- (void)build_user_interface {
	...
	if ([listClubsArray count] && ![[[listClubsArray objectAtIndex:0] objectForKey:@"phone"] isEqualToString:@""]) {
		UIButton *call_button = [UIButton buttonWithType:UIButtonTypeRoundedRect];
		[call_button setFrame:CGRectMake(130, yaxis, 170, 40)];
		[call_button setTitle:[[listClubsArray objectAtIndex:0] objectForKey:@"phone"] forState:UIControlStateNormal];
	...
	if ([listClubsArray count] && ![[[listClubsArray objectAtIndex:0] objectForKey:@"share"] isEqualToString:@""]){
	...
	if (([listHotelArray count] && (![[[listHotelArray objectAtIndex:0] objectForKey:@"address1"] isEqualToString:@""] && ![[[listHotelArray objectAtIndex:0] objectForKey:@"address2"] isEqualToString:@""])) || (![[[listClubsArray objectAtIndex:0] objectForKey:@"phone"] isEqualToString:@""])) {
		yaxis += 55;
	}
	...
	[name setText:[[listClubsArray objectAtIndex:0] objectForKey:@"name"]];
	...
	if ([[listClubsArray objectAtIndex:0] objectForKey:@"location"] == nil || [[[listClubsArray objectAtIndex:0] objectForKey:@"location"] isEqualToString:@""]) {
		...
		[add1 setTextColor:[UIColor whiteColor]];
		[add1 setBackgroundColor:[UIColor clearColor]];
		[add1 setText:[[listClubsArray objectAtIndex:0] objectForKey:@"location"]];
		[add1 setFont:[UIFont fontWithName:@"TrebuchetMS-Bold" size:18]];
		[scrollView addSubview:add1];
		yaxis += 18;
	...
	[desc setText:[[listClubsArray objectAtIndex:0] objectForKey:@"description"]];
	[desc sizeToFit];
...
-(void)downloadImage:(id)image_path {
    ...
    NSString *hotelsPath = [libraryDirectory stringByAppendingPathComponent:@"images/nightclub"];
    NSString *myFilePath = [libraryDirectory stringByAppendingPathComponent:image_path];
...
- (void)shareThis:(id)sender {
    ...
    linkUrl = [NSString stringWithFormat:@"http://www.vegashipster.com/nightclubs/%@.php",[[listHotelArray objectAtIndex:0] objectForKey:@"linkname"]];
    ...
    [AddThisSDK presentAddThisMenuForURL:linkUrl withTitle:linkTitle description:@"Check out this nightclub I've found using Vegas Hipster's iPhone app!"];
}
...
-(void)dialPhone:(id)sender {
	...
	NSScanner *phoneScanner = [NSScanner scannerWithString:[[listClubsArray objectAtIndex:0] objectForKey:@"phone"]];
...
- (void)get_gallery_count {
	NSString *NSURLString = [[NSString alloc] initWithFormat:@"http://www.vegashipster.com/mobile.php?action=gallery_count&type=clubId&id=%@", workingId];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([[segue identifier] isEqualToString:@"pushMapView"]) {
        ...
        if ([[listClubsArray objectAtIndex:0] objectForKey:@"location"] == nil || [[[listClubsArray objectAtIndex:0] objectForKey:@"location"] isEqualToString:@""]) {
            ...
            add1 = [[[listClubsArray objectAtIndex:0] objectForKey:@"location"] stringByReplacingOccurrencesOfString:@"." withString:@""];
            add1 = [add1 stringByReplacingOccurrencesOfString:@"-" withString:@""];
            add1 = [add1 stringByReplacingOccurrencesOfString:@"," withString:@""];
            add1 = [add1 stringByReplacingOccurrencesOfString:@" " withString:@"+"];
        ...
        [vc setWorkingVariables:@"clubs" withIdentifier:@"clubs_id" withId:workingId withUrl:theUrl];
        ...
        [vc setVenueType:@"clubId" andtypeId:workingId];
        ...
        [vc setTheWorkingId:[[listClubsArray objectAtIndex:0] objectForKey:@"hotel_id"]];
    }
}

//
//  VH_FriendCreateViewController.m
//  Vegas Hipster
//
...
#import "VH_FriendCreateViewController.h"

@interface VH_FriendCreateViewController ()

@end

@implementation VH_FriendCreateViewController
...
- (void)build_user_interface {
    ...
    contentRect = CGRectUnion(contentRect, group_name_lbl.frame);
    contentRect = CGRectUnion(contentRect, nameLbl.frame);
    contentRect = CGRectUnion(contentRect, phoneLbl.frame);
    contentRect = CGRectUnion(contentRect, group_name_txt.frame);
    contentRect = CGRectUnion(contentRect, nameLbl.frame);
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (!headerLabel) {
        ...
        headerLabel.text = @"Creating a Group";
    ...
    if (!textLabel) {
        ...
        textLabel.text = @"Please enter a group name and the name you want others to see when viewing you in Friend Finder.  Your phone number is optional.";
    ...
    if (!group_name_lbl) {
        group_name_lbl = [[UILabel alloc] initWithFrame:CGRectMake(10.0, 150.0, 300.0, 20.0)];
        group_name_lbl.text = @"Group Name:";
        group_name_lbl.font = [UIFont boldSystemFontOfSize:17.0];
        [group_name_lbl setBackgroundColor:[UIColor clearColor]];
        [group_name_lbl setTextColor:[UIColor whiteColor]];
        [scrollView addSubview:group_name_lbl];
    }
    
    if (!group_name_txt) {
        group_name_txt = [[UITextField alloc] initWithFrame:CGRectMake(10.0, 175.0, 300.0, 30.0)];
        group_name_txt.delegate = self;
        group_name_txt.returnKeyType = UIReturnKeyNext;
        group_name_txt.borderStyle = UITextBorderStyleRoundedRect;
        group_name_txt.tag = 1;
        [scrollView addSubview:group_name_txt];
    }
    ...
    if (!sendBtn) {
        ...
        [sendBtn addTarget:self action:@selector(create_group) forControlEvents:UIControlEventTouchUpInside];
        [sendBtn setTitle:@"Create Group" forState:UIControlStateNormal];
...
- (void)viewDidUnload {
    ...
    [group_name_lbl removeFromSuperview];
    [nameLbl removeFromSuperview];
    [phoneLbl removeFromSuperview];
    
    [group_name_txt removeFromSuperview];
...
- (void)create_group {
    NSString *post = [NSString stringWithFormat:@"action=create_group&new_app=1&group_name=%@&name=%@&phone=%@", [self urlEncodeValue:group_name_txt.text], [self urlEncodeValue:nameTxt.text], [self urlEncodeValue:phoneTxt.text]];
    ...
    if (error == nil) {
        ...
        if ([result intValue] == 1) {
            ...
            NSString *query = [[NSString alloc] initWithFormat:@"INSERT INTO ff_group (group_num, group_name, user_id) VALUES ('%@','%@','%@');",[(NSDictionary*)[response JSONValue] objectForKey:@"group_num"],group_name_txt.text,[(NSDictionary*)[response JSONValue] objectForKey:@"uid"]];

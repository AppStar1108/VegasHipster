...
#import "VH_SportsListViewController.h"

@interface VH_SportsListViewController ()

@end

@implementation VH_SportsListViewController
...
- (void)get_table_data {
	...
	NSString *query = [[NSString alloc] initWithFormat:@"SELECT sports_type_id, type FROM sports_type_db ORDER BY type ASC;"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [self performSegueWithIdentifier:@"SportsSubListPushSegue" sender:self];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([[segue identifier] isEqualToString:@"SportsSubListPushSegue"]) {
        
        // Get reference to the destination view controller
        VH_SportsSubListViewController *vc = [segue destinationViewController];
        ...
        [vc getTypeId:[[listArray objectAtIndex:selectedIndex] objectForKey:@"id"]];
    }
}

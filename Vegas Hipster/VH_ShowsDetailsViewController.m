//
//  VH_ShowsDetailsViewController.m
//  Vegas Hipster
//
...
@interface VH_ShowsDetailsViewController ()

@end

@implementation VH_ShowsDetailsViewController
...
- (void)get_table_data {
	...
	NSString *query = [[NSString alloc] initWithFormat:@"SELECT name, hotel_id, showroom, phone, description, image_path FROM shows_db WHERE shows_id = %@;", workingId];
	if (sqlite3_prepare_v2(db, [query UTF8String], -1, &statement, nil) == SQLITE_OK) {
		while (sqlite3_step(statement) == SQLITE_ROW) {
			...
			NSString *shows_location = [NSString stringWithUTF8String:(char *)sqlite3_column_text(statement, 2)];
            ...
            NSString *show_image = [NSString stringWithUTF8String:(char *)sqlite3_column_text(statement, 5)];
            
			if (show_image == NULL || [show_image isEqualToString:@""]) {
				header_image = [UIImage imageNamed:@"hdrShows.jpg"];
			}else{
				NSString *image_path = [show_image stringByReplacingOccurrencesOfString:@"../" withString:@""];
	...
	NSString *query2 = [[NSString alloc] initWithFormat: @"SELECT name, address1, address2 FROM hotel_db WHERE hotel_id = %@;", [[listLocationArray objectAtIndex:0] objectForKey:@"hotel_id"]];
...
- (void)build_user_interface {
	...
	if ([listLocationArray count] && ![[[listLocationArray objectAtIndex:0] objectForKey:@"share"] isEqualToString:@""]){
	...
	if (([listHotelArray count] && (![[[listHotelArray objectAtIndex:0] objectForKey:@"address1"] isEqualToString:@""] && ![[[listHotelArray objectAtIndex:0] objectForKey:@"address2"] isEqualToString:@""])) || (![[[listLocationArray objectAtIndex:0] objectForKey:@"phone"] isEqualToString:@""])) {
		yaxis += 55;
	}
	...
	if ([[listLocationArray objectAtIndex:0] objectForKey:@"location"] == nil || [[[listLocationArray objectAtIndex:0] objectForKey:@"location"] isEqualToString:@""]) {
		...
		[add1 setTextColor:[UIColor whiteColor]];
		[add1 setBackgroundColor:[UIColor clearColor]];
		[add1 setText:[[listLocationArray objectAtIndex:0] objectForKey:@"location"]];
		[add1 setFont:[UIFont fontWithName:@"TrebuchetMS-Bold" size:18]];
		[scrollView addSubview:add1];
		yaxis += 18;
...
- (void)shareThis:(id)sender {
    ...
    linkUrl = [NSString stringWithFormat:@"http://www.vegashipster.com/entertainment/%@.php",[[listLocationArray objectAtIndex:0] objectForKey:@"linkname"]];
	linkTitle = [NSString stringWithFormat:@"You should check out %@!",[[listLocationArray objectAtIndex:0] objectForKey:@"name"]];
    
    [AddThisSDK setFavoriteMenuServices:@"facebook",@"twitter",nil];
    
    [AddThisSDK setAddThisPubId:@"ra-51ffe583468b203f"];
    
    [AddThisSDK presentAddThisMenuForURL:linkUrl withTitle:linkTitle description:@"Check out this show I've found using Vegas Hipster's iPhone app!"];
}
...
-(void)downloadImage:(id)image_path {
    ...
    NSString *hotelsPath = [libraryDirectory stringByAppendingPathComponent:@"images/shows"];
    NSString *myFilePath = [libraryDirectory stringByAppendingPathComponent:image_path];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([[segue identifier] isEqualToString:@"pushMapView"]) {
        ...
        [vc setWorkingVariables:@"shows_db" withIdentifier:@"shows_id" withId:workingId withUrl:theUrl];

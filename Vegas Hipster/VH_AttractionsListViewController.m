...
@interface VH_AttractionsListViewController ()

@end

@implementation VH_AttractionsListViewController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (!imageView) {
        ...
        imageView.image = [UIImage imageNamed:@"hdrAttractions.jpg"];
...
- (void)get_table_data {
    ...
    query = [NSString stringWithFormat:@"SELECT count(*) as count FROM attractions_db JOIN prices ON attractions_db.attractions_id = prices.table_id WHERE prices.table_name = 'attractions_db';"];
    ...
    query2 = [NSString stringWithFormat:@"SELECT attractions_db.attractions_id, attractions_db.name, prices.value FROM attractions_db JOIN prices ON attractions_db.attractions_id = prices.table_id WHERE prices.table_name = 'attractions_db'  ORDER BY attractions_db.name ASC Limit %d, 50;", start];    
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([[segue identifier] isEqualToString:@"AttractionsDetailsPushSegue"]) {

//
//  VH_ClubsSubListViewController.m
//  Vegas Hipster
//
...
#import "VH_ClubsSubListViewController.h"

@interface VH_ClubsSubListViewController ()

@end

@implementation VH_ClubsSubListViewController
@synthesize clubType;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (!imageView) {
        ...
        imageView.image = [UIImage imageNamed:@"hdrClubs.jpg"];
...
- (void)get_table_data {
    ...
    if ([clubType isEqualToString:@"9"]) {
        query = [[NSString alloc] initWithFormat:@"Select count(*) as count From clubs Where clubs_id In (SELECT clubs.clubs_id FROM clubs JOIN prices ON clubs.clubs_id = prices.table_id WHERE prices.table_name = 'clubs'  ORDER BY clubs.name ASC)"];
	}else{
        query = [[NSString alloc] initWithFormat:@"Select count(*) as count From clubs Where clubs_id In (SELECT clubs.clubs_id FROM clubs JOIN prices ON clubs.clubs_id = prices.table_id WHERE clubs.club_type = %@ And prices.table_name = 'clubs'  ORDER BY clubs.name ASC)", clubType];
    }
    ...
    if ([clubType isEqualToString:@"9"]) {
        query2 = [NSString stringWithFormat:@"SELECT clubs.clubs_id, clubs.name, prices.value FROM clubs JOIN prices ON clubs.clubs_id = prices.table_id WHERE prices.table_name = 'clubs'  ORDER BY clubs.name ASC Limit %d, 50;", start];
	}else{
        query2 = [NSString stringWithFormat:@"SELECT clubs.clubs_id, clubs.name, prices.value FROM clubs JOIN prices ON clubs.clubs_id = prices.table_id WHERE clubs.club_type = %@ And prices.table_name = 'clubs'  ORDER BY clubs.name ASC Limit %d, 50;", clubType, start];
    }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([[segue identifier] isEqualToString:@"ClubsDetailsPushSegue"]) {

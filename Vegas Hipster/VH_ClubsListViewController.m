...
#import "VH_ClubsListViewController.h"

@interface VH_ClubsListViewController ()

@end

@implementation VH_ClubsListViewController
...
- (void)get_table_data {
	...
	NSString *query = [[NSString alloc] initWithFormat:@"SELECT club_type_id, type FROM club_type_db ORDER BY type ASC;"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [self performSegueWithIdentifier:@"ClubsSubListPushSegue" sender:self];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([[segue identifier] isEqualToString:@"ClubsSubListPushSegue"]) {
        
        // Get reference to the destination view controller
        VH_ClubsSubListViewController *vc = [segue destinationViewController];
        ...
        [vc getClubType:[[listArray objectAtIndex:selectedIndex] objectForKey:@"id"]];
    }
}

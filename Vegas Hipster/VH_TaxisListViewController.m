...
#import "VH_TaxisListViewController.h"

@interface VH_TaxisListViewController ()

@end

@implementation VH_TaxisListViewController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (!imageView) {
        ...
        imageView.image = [UIImage imageNamed:@"hdrTaxi.jpg"];
...
- (void)get_table_data {
    ...
    query = [NSString stringWithFormat:@"SELECT count(*) as count FROM taxi_db;"];
    ...
    query2 = [NSString stringWithFormat:@"SELECT Taxi_db.Taxi_id, Taxi_db.taxi_name FROM Taxi_db ORDER BY Taxi_db.taxi_name ASC Limit %d, 50;", start];    
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if ([pagination isEqualToString:@"Previous"]){
        ...
        [self performSegueWithIdentifier:@"TaxisDetailsPushSegue" sender:self];
	}
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([[segue identifier] isEqualToString:@"TaxisDetailsPushSegue"]) {
        // Get reference to the destination view controller
        VH_TaxisDetailsViewController *vc = [segue destinationViewController];

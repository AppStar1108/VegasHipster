//
//  VH_SportsDetailsViewController.m
//  Vegas Hipster
//
...
@interface VH_SportsDetailsViewController ()

@end

@implementation VH_SportsDetailsViewController
...
- (void)get_table_data {
	...
	NSString *query = [[NSString alloc] initWithFormat:@"SELECT name, hotel_id, showroom, phone, description, image_path FROM sports_db WHERE sports_id = %@;", workingId];
	if (sqlite3_prepare_v2(db, [query UTF8String], -1, &statement, nil) == SQLITE_OK) {
		while (sqlite3_step(statement) == SQLITE_ROW) {
			...
			if (show_image == NULL || [show_image isEqualToString:@""]) {
				header_image = [UIImage imageNamed:@"hdrSports.jpg"];
			}else{
...
- (void)shareThis:(id)sender {
    ...
    [AddThisSDK presentAddThisMenuForURL:linkUrl withTitle:linkTitle description:@"Check out this sports attraction I've found using Vegas Hipster's iPhone app!"];
}
...
-(void)downloadImage:(id)image_path {
    ...
    NSString *hotelsPath = [libraryDirectory stringByAppendingPathComponent:@"images/sports"];
    NSString *myFilePath = [libraryDirectory stringByAppendingPathComponent:image_path];

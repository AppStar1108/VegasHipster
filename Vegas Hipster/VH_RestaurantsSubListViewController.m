//
//  VH_RestaurantsSubListViewController.m
//  Vegas Hipster
//
...
@interface VH_RestaurantsSubListViewController ()

@end

@implementation VH_RestaurantsSubListViewController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (!imageView) {
        ...
        imageView.image = [UIImage imageNamed:@"hdrDining.jpg"];
...
- (void)get_table_data {
    ...
    if ([categoryType isEqualToString:@"1"]) {
		query = [NSString stringWithFormat:@"SELECT count(*) as count FROM rest_db JOIN prices ON rest_db.rest_id = prices.table_id WHERE prices.table_name = 'rest_db';"];
	}else{
        query = [NSString stringWithFormat:@"SELECT count(*) as count FROM rest_db JOIN prices ON rest_db.rest_id = prices.table_id WHERE rest_db.rest_type = %@ And prices.table_name = 'rest_db';", categoryType];
    }
    ...
    if ([categoryType isEqualToString:@"1"]) {
		query2 = [NSString stringWithFormat:@"SELECT rest_db.rest_id, rest_db.name, prices.value FROM rest_db JOIN prices ON rest_db.rest_id = prices.table_id WHERE prices.table_name = 'rest_db'  ORDER BY rest_db.name ASC Limit %d, 50;", start];
	}else{
        query2 = [NSString stringWithFormat:@"SELECT rest_db.rest_id, rest_db.name, prices.value FROM rest_db JOIN prices ON rest_db.rest_id = prices.table_id WHERE rest_db.rest_type = %@ And prices.table_name = 'rest_db'  ORDER BY rest_db.name ASC Limit %d, 50;", categoryType, start];
    }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([[segue identifier] isEqualToString:@"RestaurantDetailsPushSegue"]) {
